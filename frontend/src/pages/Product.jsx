    ...
    <div className="border-t-2 pt-10 transition-opacity ease-in duration-500 opacity-100">
      <div className="flex gap-12 flex-col sm:flex-row">
        <div className="flex-1 flex flex-col-reverse gap-3 sm:flex-row">
          <div className="flex sm:flex-col overflow-x-auto justify-between">
            {productData.image.map((item, index) => (
              <img
                onClick={() => setImage(item)}
                src={item}
                key={index}
                className="w-[50%] sm:mb-3 flex-shrink-0 cursor-pointer"
                alt=""
              />
            ))}
          </div>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-1 mt-3">
            <IoStar className="w-5" />
            <IoStar className="w-5" />
            <IoStar className="w-5" />
            <IoStarHalf className="w-5" />
      ...
      <RelatedProducts category={productData.category} subCategory={productData.subCategory} id={productData.id} />
    </div>
  ) : (
